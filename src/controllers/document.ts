...
export default class document {
  ...
  static createDocumentController = async (
    ...
    try {
      ...
      await dbServices.document.createDocument(
        UserId,
        cleanedArticle,
        metadata,
        keyword
  ...
  static getDocumentById = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (result.length == 0)
        res.status(500).send({ statys: false, messsage: "Data Not Found..." });
      res
        .status(200)
        .send({ status: true, message: "Get Documnet Successfully", result });
    } catch (error: any) {
      res
        .status(500)
        .json({ status: false, error: "Error in getting Document" });
    }
  };
  ...
  static updateDocument = async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (!content)
        res.status(500).send({ message: "content Not Found", status: false });
      const updateDoc = await dbServices.document.updateDoc(
        userId,
        parseInt(documentId),
        content
      );
